pipeline{
  ...
  environment {
    DEPLOYMENT_NAME = "frontend-speedtest-web"
    IS_FULL = "${params.IS_FULL}"
    MERCHANT_NO = "${params.MERCHANT_NO}"
    IMAGE_NAME = "369181834085.dkr.ecr.ap-east-1.amazonaws.com/acebet-speedtest-web:${MERCHANT_NO}-${BRANCH_NAME}.${BUILD_ID}"
  ...
  stages {
    stage('Full deploy to AWS') {
      ...
      steps {
        ...
        sed -i s#branchname#${BRANCH_NAME}#g ./cm/merchant/frontend/speedtest-web.yml
        kubectl -n merchant-${MERCHANT_NO} apply -f ./cm/merchant/frontend/speedtest-web.yml
        rm -rf ./cm
        '''
      }
    }
    stage('Build dist') {
      steps {
        sh '''
        echo "---Build dist---"
        curl -u ${APOLLO_AUTH} ${APOLLO_URL}/acebet.frontend/${MERCHANT_NO}/application,speedtest-web -o .env.production -s
        sed -i s#{projectname}#speedtest-web#g .env.production
        npm install
        npm run build
        '''
      }
    ...
    stage('Sync to BOS') {
      steps {
        ...
        bce bos sync dist bos:/acebet-assets/${merchant_hash}/speedtest-web --exclude './*.html' --exclude './*.htm' --yes
        '''
      }
    }
