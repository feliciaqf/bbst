  ...
  <div class="home">
    <div class="header">
      <layout justify-space-between align-center>
        <div>
          <img v-if="siteinfo" class="logo" :src="siteinfo.logo" alt="">
          <!-- <span v-if="$projectName !== 'chobet'&&!aoMenUi.includes($projectName)">官方战略合作</span> -->
        </div>
        <div v-if="$projectName === '10031'" class="header-right">
          <img :src="$imgs['logo-right']" alt="">
        </div>
    ...
    <div class="bg">
      <div class="content">
        ...
        <div class="tips">温馨提示：MS值越小,打开网站速度越快,越排前的网址越快！</div>
        <div class="list">
          <template v-if="lists.length > 0">
            <layout class="item" v-for="(item, $index) in lists" :key="$index">
              <layout align-center class="left">
                <div :class="['time', getClass(item.speed)]">{{item.speed}}<span v-show="item.speed !== 'error'">ms</span></div>
                <div class="domain">备用域名{{$index}}：{{item.domain.domain}}</div>
              </layout>
              <div class="button" @click="open(item)">进入游戏</div>
            </layout>
          </template>
          <template v-else-if="lists.length == 0">
            <layout class="item empty" v-for="(item, $index) in 4" :key="$index">
              <layout align-center class="left">
        ...
        <div :class="['retest',{'disabled':testBtnShow}]" @click="backDomain" onselectstart="return false">重新测速</div>
      </div>
...
let AppFooter = {}
if (
  process.env.VUE_APP_PROJECT_NAME === '10033' ||
  process.env.VUE_APP_PROJECT_NAME === '10063' ||
  process.env.VUE_APP_PROJECT_NAME === '10029' ||
  process.env.VUE_APP_PROJECT_NAME === '10059' ||
  process.env.VUE_APP_PROJECT_NAME === '10028' ||
  process.env.VUE_APP_PROJECT_NAME === '10023' ||
  process.env.VUE_APP_PROJECT_NAME === '10022' ||
  process.env.VUE_APP_PROJECT_NAME === '10025' ||
  process.env.VUE_APP_PROJECT_NAME === '10060' ||
  process.env.VUE_APP_PROJECT_NAME === '10026' ||
  process.env.VUE_APP_PROJECT_NAME === '10027' ||
  process.env.VUE_APP_PROJECT_NAME === '10043' ||
  process.env.VUE_APP_PROJECT_NAME === '10053' ||
  process.env.VUE_APP_PROJECT_NAME === '10054' ||
  process.env.VUE_APP_PROJECT_NAME === '10052' ||
  process.env.VUE_APP_PROJECT_NAME === '10055' ||
  process.env.VUE_APP_PROJECT_NAME === '10056' ||
  process.env.VUE_APP_PROJECT_NAME === '10031'
) {
  AppFooter = require(`@/components/${process.env.VUE_APP_PROJECT_NAME}/footer/index`).default
} else if (process.env.VUE_APP_PROJECT_NAME === '10024') {
...
export default {
  ...
  data () {
    return {
      ...
      lists: [],
      testBtnShow: 'false',
      aoMenUi: ['takchun', '10012', '10013', '10020', '10018', '10014', 'mgm8', '10006', '10011', '10007', '10009', '10010', '10008', '10021', '10034']
    }
  },
  methods: {
    getClass (time) {
...
.home {
  ...
  .content {
    ...
    .retest {
      ...
      margin: 20px auto 0;
      cursor: pointer;
      background:linear-gradient(90deg,rgba(130,166,255,1) 0%,rgba(43,113,247,1) 100%);
      box-shadow:0px 4px 14px 0px rgba(52,118,248,0.4);
      border-radius:25px;
      &:hover {
        // background: #525152;
      }
